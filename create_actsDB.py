...
collection = create_milvus_collection('acts2_db', 384)
...
for new_csv in tqdm(pd.read_csv("CommercialActs.csv",chunksize=chunksize,encoding="utf-8")):
    counter = 0
    embed_chunks=[]
    embeded_chunks=[]
    chunk_id_reference = []
    reference = "Name of statute: "+new_csv["Name of statute"]+ " " +"Section Number: "+new_csv["Section Number"]+ " " +"Section Title: "+new_csv["Section Title"]
    references = "Name of statute: "+new_csv["Name of statute"]+ " " +"Section Number: "+new_csv["Section Number"]+ " " +"Section Title: "+new_csv["Section Title"]+ " " +"Section Text: "+new_csv["Section Text"]
    new_csv=new_csv['Section Text']
    ...
    for chunks in text_chunks:
        for i in range(0,len(references)):
            if chunks.metadata["row"] == i + length:
                if len(references.iloc[i])>50000:
                    chunks.metadata["reference"] = references.iloc[i][:50000]
                    embeded_chunks.append(chunks)
                else:
                    chunks.metadata["reference"] = references.iloc[i]
                    embeded_chunks.append(chunks)
    ...
    # print(length)
    # print(len_of_text)
    # print(f"lenghth of embed_chunks: {len(embed_chunks)}")
    ...
    dict_list = []
    for chunk, chunks, vector, ids in zip(embed_chunks, embeded_chunks, embeddings, ids_arr):
